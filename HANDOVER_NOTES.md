...
37. **AI generator resilience + the real "high demand" cause + FULL MOBILE DASHBOARD PARITY.** (a) Fixes: cascade expanded to observed-healthy models; per-model retry with backoff; 75s attempt timeout. (b) Raw JSON leak fixed — provider failures map to human copy. (c) Frontend: exam-gen axios 120s timeout + "may take ~1 min" label. (d) Mobile dashboard parity: shared `navLinks` + slide-in drawer with yellow hamburger on phones; all role features incl. Exam Management reachable on mobile. Validation: ESLint clean, `vite build` OK. Note: 30s per-author cooldown protects the server key's quota.

38. **Sitewide language translation — every page readable in 15 languages (NEW, deployed with frontend `1659c86`).** Free stack, zero new deps/env keys: NEW `src/components/common/LanguageSwitcher.jsx` (custom premium dropdown driving the Google Translate widget via its `googtrans` cookie + hidden host element; Google's own banner/chrome CSS-hidden in `src/index.css`). Placement: Navbar (desktop) + Footer bottom bar (all devices incl. phones). Choice persists 1 year (cookie + localStorage `chub_language`); switching reloads once so the saved language applies to every page, dashboard included. `src/main.jsx` hardened: the extension-error sweeper allow-lists `goog-te-*`/`skiptranslate` nodes so it can never strip the translator. Languages: EN + French, Hausa, Igbo, Yoruba, Arabic, Spanish, Portuguese, German, Italian, Chinese, Hindi, Kiswahili, Amharic, Dutch. Validation: ESLint clean, `vite build` OK (~1 min). Note: English remains the source/authoring language; translations are machine-generated on the fly.
