...
const fetchCourses = async ({ queryKey }) => {
  ...
  try {
    ...
    console.log("REAL API SUCCESS:", res.data);
    return res.data;
  } catch (error) {
    console.warn("Falling back to local courses data:", error);
    return {
      courses: coursesFallback.filter((course) =>
...
const AllCourses = () => {
  ...
  console.log("RENDER STATE:", { data, isLoading, isError, error });
